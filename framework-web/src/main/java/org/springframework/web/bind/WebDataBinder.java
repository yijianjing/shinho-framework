...
/**
 * 在 doBind 方法中增加 checkFieldByObjectName,支持对象名.属性名封装
 *
...
public class WebDataBinder extends DataBinder {
	...
	protected void doBind(MutablePropertyValues mpvs) {
		...
		//支持对象名.属性名封装
		checkFieldByObjectName(mpvs);
		super.doBind(mpvs);
	}

	protected void checkFieldByObjectName(MutablePropertyValues mpvs) {

		String fieldPrefix = getObjectName()+".";
		PropertyValue[] pvArray = mpvs.getPropertyValues();
		for (PropertyValue pv : pvArray) {
			if (pv.getName().startsWith(fieldPrefix)) {
				String field = pv.getName().substring(fieldPrefix.length());
